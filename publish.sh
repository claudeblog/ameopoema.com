...
RSS_BUTTON='<button onclick="(function(){const u='\''https://ameopoema.com/feed'\''; navigator.clipboard.writeText(u).then(()=>{const t=document.createElement('\''div'\''); t.textContent='\''📋 ameopoema.com/feed copiado'\''; t.style.cssText='\''position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#333;color:#fff;padding:10px 20px;border-radius:6px;z-index:9999;opacity:0;transition:opacity 0.3s'\''; document.body.appendChild(t); setTimeout(()=>t.style.opacity='\''1'\'',10); setTimeout(()=>{t.style.opacity='\''0'\''; setTimeout(()=>t.remove(),300)},2000)}).catch(()=>alert('\''Erro ao copiar'\''))})()" ...>RSS</button>'
